...
export default function MessagesPage() {
  const [practitionerId] = useState<string | null>(() =>
  ...
  const fetchConversations = useCallback(async () => {
    ...
    try {
      const res = await api.get(`/chat/conversations/${practitionerId}`);
      const data = res.data;
      ...
      const mapped: Conversation[] = data.map((conv: Record<string, unknown> & { patientId?: { firstName?: string; lastName?: string } }) => ({
        _id: conv._id as string,
        name: conv.patientId?.firstName
          ? `${conv.patientId.firstName} ${conv.patientId.lastName}`
          : "Patient",
        unread: (conv.unreadForPractitioner as number) || 0,
      }));
  ...
  const fetchAllPatients = async () => {
    ...
    try {
      ...
      const data = res.data;
      if (Array.isArray(data)) setAllPatients(data as Patient[]);
    } catch {
      setAllPatients([]);
    } finally {
  ...
  const startNewChat = async (patientId: string) => {
    ...
    try {
      ...
      // Failed to start chat — user can retry
    }
  };
  ...
  useEffect(() => {
    ...
    if (practitionerId) {
      api
        .post(`/chat/reset-unread/${selected._id}`, { role: "practitioner" })
        .catch(() => {});
    }
  ...
  }, [selected, socket, practitionerId]);
  ...
  // Suppress unused warning — onlineUsers drives presence indicators later
  void onlineUsers;
  ...
  return (
    <div className="h-[calc(100vh-100px)] w-full flex bg-bg-page overflow-hidden">
      {/* LEFT PANEL */}
      <div
        className={`${selected ? "hidden md:flex" : "flex"} w-full md:w-1/3 lg:w-1/4 bg-bg-card border-r border-border flex-col transition-all duration-300`}
      >
      ...
      {/* RIGHT PANEL */}
      <div
        className={`${selected ? "flex" : "hidden md:flex"} flex-1 flex-col bg-bg-page relative`}
      >
        {!selected ? (
            ...
            <div className="p-4 border-b border-border flex items-center justify-between bg-bg-card shadow-sm z-10 sticky top-0">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-text-secondary mt-0.5">Patient</p>
                </div>
              </div>
            </div>
              ...
              {Object.entries(groupedMessages).map(([date, msgs]) => (
                <div key={date} className="animate-fadeInUp">
                  ...
                  <div className="space-y-3">
                    {msgs.map((msg, index) => {
                      const isMe = msg.senderId === practitionerId;
                      const showAvatar =
                        index === 0 ||
                      ...
                      return (
                        ...
                        >
                          {!isMe && (
                            <div
                              className={`w-8 h-8 rounded-full shrink-0 flex items-center justify-center font-medium text-xs ${showAvatar ? "bg-primary-light text-primary visible" : "invisible"}`}
                            >
                              {selected.name?.charAt(0) || "P"}
                            </div>
                          )}
                          <div
                            className={`relative px-4 py-2.5 rounded-2xl max-w-[75%] sm:max-w-[70%] text-[15px] leading-relaxed shadow-sm ${
                              isMe
                          ...
                          >
                            {msg.text}
                            <div
                              className={`text-[10px] mt-1 flex items-center gap-1.5 ${isMe ? "text-white/80 justify-end" : "text-text-secondary justify-start"}`}
                            >
                              <span>
              ...
              {typing && (
                <div className="flex justify-start gap-2 animate-fadeInLeft">
                  ...
                  <div className="bg-bg-card border border-border px-4 py-3 rounded-2xl rounded-tl-sm shadow-sm flex gap-1.5 items-center">
                    <div
                      className="w-2 h-2 rounded-full bg-text-disabled animate-bounce"
