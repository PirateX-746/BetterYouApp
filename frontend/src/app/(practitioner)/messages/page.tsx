...
export default function MessagesPage() {
    ...
    console.log('[Practitioner] Render. practitionerId:', practitionerId, 'selected:', selected?._id);
    ...
    const fetchConversations = async () => {
        ...
        try {
            ...
            const data = res.data;
            console.log('[Practitioner] raw conversations data:', data);
            ...
            console.error("[Practitioner] Error fetching conversations:", err);
        }
    };
    ...
    const fetchMessages = async (conversationId: string) => {
        try {
            ...
            console.error("[Practitioner] Error fetching messages:", err);
        }
    };
    ...
    const fetchAllPatients = async () => {
        ...
        try {
            ...
            console.error("[Practitioner] Error fetching patients:", err);
        } finally {
    ...
    const startNewChat = async (patientId: string) => {
        ...
        try {
            const res = await api.post(`/chat/conversation`, { patientId, practitionerId });
            const conv = res.data;
            ...
            console.error("[Practitioner] Error starting new chat:", err);
        }
    };
    ...
    useEffect(() => {
        ...
        if (practitionerId) {
            api.post(`/chat/reset-unread/${selected._id}`, {
                role: "practitioner"
            }).catch(err => console.error("[Practitioner] Error resetting unread:", err));
        }
                    ...
                    <div className="bg-bg-card rounded-2xl w-full max-w-md p-6 max-h-[85vh] flex flex-col shadow-xl animate-scaleIn">
                        <div className="flex justify-between items-center mb-6">
                            <div>
                                ...
                                <p className="text-sm text-text-secondary mt-1">Select a patient to message</p>
                            </div>
                            <button
                        ...
                        <div className="flex-1 overflow-y-auto space-y-2 pr-2 custom-scrollbar">
                            {isLoadingPatients ? (
                                ...
                                <div className="text-center py-10">
                                    ...
                                    <p className="text-text-primary font-medium">No patients found</p>
                                    <p className="text-text-secondary text-sm">You dont have any patients yet.</p>
                                </div>
                            ) : (
                                allPatients.map((p) => (
                                    <div
                                    ...
                                    >
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <p className="font-semibold text-text-primary truncate">{p.firstName} {p.lastName}</p>
